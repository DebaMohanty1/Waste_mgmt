...
def main():
    ...
    if uploaded_file is not None:
        ...
        if not os.path.exists(model_weights_path):
            st.error(f"Required model weights file 'model_final.pth' not found. Please download it from: {GOOGLE_DRIVE_LINK}")
            st.stop()
        ...
        st.image(result_image, caption="Input and Output", use_column_width=True)

        def dpi_mm(uploaded_file,x):
            metadata=Image.open(uploaded_file).info
        ...
        st.write(f"The contamination percent is \t{round((nbd_mm / bin_mm) * 100, 0)} %")
        st.write("*" * 40)
